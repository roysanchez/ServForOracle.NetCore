...
namespace ServForOracle.NetCore
{
    ...
    {
        public static async Task ExecuteProcedureAsync<A>(this ServForOracle s, string procedure, A p1)
            => await s.ExecuteProcedureAsync(procedure, Param.Input(p1));
        public static async Task ExecuteProcedureAsync<A, B>(this ServForOracle s, string procedure, A p1, B p2)
            => await s.ExecuteProcedureAsync(procedure, Param.Input(p1), Param.Input(p2));
        public static async Task ExecuteProcedureAsync<A,B,C>(this ServForOracle s, string procedure, A p1, B p2, C p3)
            => await s.ExecuteProcedureAsync(procedure, Param.Input(p1), Param.Input(p2), Param.Input(p3));
        public static async Task ExecuteProcedureAsync<A,B,C,D>(this ServForOracle s, string procedure, A p1, B p2, C p3, D p4)
